...
export function show(tab) {
  ...
  if (activeTab === 'global') { renderGlobalTab(meta); return; }
    ...
    wrap.appendChild(el('div', { class: 'upg-row global-row' }, [
      el('div', { class: 'upg-head' }, [
        ...
        el('div', { class: 'upg-info' }, [
          el('b', { class: 'upg-name-wrap' }, [
            el('span', { text: def.name }),
            el('span', { class: 'upg-layer global', text: 'GLOBAL' }),
          ]),
          el('span', { class: 'upg-desc', text: tr(def.desc) }),
          el('span', { class: 'upg-now', text: maxed ? `${STAT_FMT[def.stat](now)} (maks)` : `${STAT_FMT[def.stat](now)} → ${STAT_FMT[def.stat](next)}` }),
        ]),
      ]),
      slider,
      el('div', { class: 'upg-foot' }, [
...
function renderPasukanTab(meta) {
  ...
  const card = el('div', { class: 'card hero-lab-card' }, [
    ...
    el('div', { class: 'hl-ally-row' }, [
      ...
      el('img', { class: 'hl-ally', src: spriteToDataURL('assets/sprites/mako_v2/mako_stage0_idle.png'), alt: 'Mako' }),
    ]),
