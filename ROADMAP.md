...
| `BUILD` `50a → 51a → … → 60a`, `index.html ?v=60a` (JS **dan CSS**) | aturan cache-busting repo; sejak 59a tautan `main.css`/`dashboard-focus.css` ikut `?v=BUILD` |
...
| `comeback-system.js` **sudah dipakai runtime**; `pricing-model.js` dipakai validator katalog; `session-hook.js` **dipakai runtime sejak Fase 2.4** (lewat adapter). Sisa 1 modul murni masih kode mati: `rare-drop-system.js` (Fase 4.2) | `main.js:boot()`, `tools/validate-catalog.mjs`, `gameover-screen.js` + `session-hook-adapter.js` |
| **§6 ✅ Navigasi diringkas** — 4 sistem (dock, sidebar, quick row, 2 menu HUD) → **2**: dock permanen **5 slot** (slot 1 "Perjalanan") + **satu sheet Perjalanan** (`journey-sheet.js`) untuk dashboard & HUD; sidebar/quick row/secondary dock dihapus; gerbang fail-closed `JOURNEY_GATES` (9 destinasi → `secondary/*` & `dock/*`); badge unlock jadi satu; **topbar permanen 3 indikator**: chip Pangkat + chip Battle Pass + item teratas session hook; chip countdown 2.5 pindah ke topbar (dash-scroll tersembunyi sejak F24); **bug laten CSS cache-busting diperbaiki** (`?v=` CSS kini = BUILD) | `index.html`, `journey-sheet.js` (baru), `feature-gate.js`, `unlock-badge-system.js`, `main.js`, `dashboard-screen.js`, `screen-manager.js`, `input-handler.js`, `features.json` (23→11 gate), `dashboard-focus.css`, `lang.json` +1, **43 pemeriksaan headless** (`npm run test:fasenav`) |
| **F3 ✅ Monetisasi tuntas** — penanda UI **bonus 2× pembelian pertama** (imun_500/1000, hilang setelah terpakai); **sisa hari Kartu Imun** di chip topbar dashboard + baris profil (+klaim); **dua sink Imun**: Lanjut Run **50 Imun** (1/run, `confirmRevive` yang sama, di luar kuota iklan) & Peti Riset **150 Imun** (2× loot, penanda terpisah dari jalur iklan) — biaya di `economy-anchors.json:imunSinks` (baru) via `getImunSinks()`; **audit bebas iklan** (tabel §7; G15 tetap terbuka) + **bug fix `hasPerk`** (perk permanen `0` ditolak falsy); kurs & produk nonaktif dipin test | `game.js`, `revive-screen.js`, `bosschest-screen.js`, `shop-screen.js`, `dashboard-screen.js`, `profile-screen.js`, `imun-economy.js`, `data-store.js`, `economy-anchors.json`, `index.html`, `main.css`, `dashboard-focus.css`, `lang.json` +4, **54 pemeriksaan headless** (`npm run test:fase3`) |
| **2.4 ✅ Hook akhir sesi** — maks 3 baris "alasan main lagi" **di atas** tombol Main Lagi, ETA dalam **run** (`retention-config.sessionHook`: prioritas, `minPctToShow` 0,35, `maxEtaRuns` 6, `maxItems` 3). Drop-in `session-hook.js` **tidak diedit**: skema repo dijembatani `session-hook-adapter.js` (G1 hero, G2 arena, G3 pangkat, G4 bab, G5 evolusi, G7 komposisi `retention`, G12 `meta.bp`); label evolusi dipermanis ke nama bagian; try/catch — hook tidak pernah menjatuhkan layar gameover | `gameover-screen.js` (`renderSessionHook`), `js/systems/session-hook-adapter.js` (baru), `main.css` (.go-hook), `lang.json` +1, **54 pemeriksaan headless** (`npm run test:fase2hook`) |
| **2.5 ✅ Hitung mundur reset harian** — chip `HH:MM:SS` di tiga surface dari `dailyAnchor.countdownSurfaces` (daftar misi dashboard, kartu gameover, panel Misi HUD); batas = **tengah malam UTC berikutnya**, diturunkan dari konvensi kunci tanggal UTC semua fitur harian; sisa < `warnWhenHoursLeft` (4, data) → penekanan coral berdenyut | `js/systems/daily-reset.js` + `js/ui/reset-countdown.js` (baru), `dashboard-screen.js`, `gameover-screen.js`, `hud-screen.js`, `main.css`, 3 string `lang.json`, 40 pemeriksaan headless (`npm run test:fase2count`) |
| **2.1+2.3 ✅ Onboarding** — auto-fire default NYALA (model Archero, **direvisi 13 Sep BUILD 58a atas evaluasi pemilik: menembak sendiri HANYA SAAT DIAM** — joystick dilepas; bergerak = menghindar tanpa menembak; aim manual/tahan SERANG mengambil alih kapan pun termasuk sambil bergerak; pasukan mengikuti ritme tembakan hero; tanpa target hero diam) + toggle "Serang Otomatis" di Profil; gerbang landscape kini **hanya** saat gameplay (`body.in-run`) | `game.js`, `profile-screen.js`+`index.html`, `hud-screen.js` (hint adaptif "lepas = menembak saat diam"), `main.css`, `state-manager.js` (`settings.autoFire`), 21 pemeriksaan headless |
...
| `rare-drop-system.js` **masih kode mati** | drop kosmetik langka (Fase 4.2) belum ada; menunggu keputusan **G6** (`f2pDroppable` belum ada di `cosmetics.json`) dan **G13** (bentuk kanonik `meta.cosmetics.skin`) |
...
Angka repo yang terverifikasi hari ini: **11 hero**, 13 tipe musuh, 5 arena, 6 bab kampanye, 13 tier pangkat, 5 skin + 2 aksesori, 35 file `data/*.json`, 106 file `js/*.js` (46 sistem, 23 layar).
...
|---|---|---|---|
| **G1** | `hero.unlock.stats` = peta `{statKey: need}` (`session-hook.js:88-90`) | `heroes.json`: `unlock = {type:"stat", stat:"totalRuns", value:15, label}` | `gates` = undefined → **baris `unlock_hero` tidak pernah muncul** **STATUS 13 Sep: ✅ selesai lewat adapter pemanggil (Fase 2.4)** — `session-hook-adapter.adaptHero()`: unlock `type:'stat'` → `{stats:{stat:value}}`; unlock `imu`/`default` dilepas karena bukan gerbang statistik. Diuji headless (`test:fase2hook`) |
| **G2** | `arena.unlock` = peta `{statKey: need}` (`session-hook.js:107`) | `arenas.json`: `unlock = {type:"default", value:0}` | iterasi menghasilkan kunci `type`/`value` → **baris `unlock_arena` sampah/tak muncul** **STATUS 13 Sep: ✅ selesai lewat adapter (Fase 2.4)** — `adaptArena()`: `{type:'<statKey>',value}` → peta datar `{statKey:value}` (fromArenaUnlocks mengiterasi `unlock` langsung, bukan `.stats`); `type:'default'` → unlock dilepas agar tidak jadi baris sampah |
| **G3** | `tier.gp` (`session-hook.js:145-147`) | `ranks.json`: tier memakai **`min`** (`rank-system.js:33,44` sudah benar pakai `.min`) | `undefined > gp` selalu false → **baris `pangkat_tier` tidak pernah muncul** **STATUS 13 Sep: ✅ selesai lewat adapter (Fase 2.4)** — `adaptRankTier()`: `gp = min`, field asli utuh |
| **G4** | `ch.quota`, `ch.name` (`session-hook.js:184-190`) | `campaign.json`: **`killQuota`**, `organ`/`title` (tidak ada `name`) | **baris `kuota_bab` tidak pernah muncul** **STATUS 13 Sep: ✅ selesai lewat adapter (Fase 2.4)** — `adaptChapter()`: `quota = killQuota`, `name = organ || title` |
| **G5** | id bagian `equity_membran`, `inti_memori` (`retention-config.json`) | `evolutions.json`: **`equity_membrane`**, **`equity_memory_core`** | **SELESAI 13 Sep (Fase 1.4) tanpa rename:** biaya tahap sudah 50/50/50/17 = **167**; id repo dipertahankan karena rename akan memutasi `meta.evoParts` di save pemain, jalur sprite, dan rujukan Bio-Pedia. `tools/validate-retune-sync.mjs` membandingkan **total fragmen per bagian** lewat peta alias `{equity_membran→equity_membrane, inti_memori→equity_memory_core}`. **Sisa:** ~~`fromEvolution` di `session-hook.js` masih salah kunci sampai Fase 2.4 memakai adapter~~ **✅ selesai 13 Sep (Fase 2.4):** `remapStageCost()` di adapter menerjemahkan kunci config → id repo saat komposisi `data.retention`; label "Evolusi — <id teknis>" dipermanis pemanggil jadi nama bagian dari `evolutions.json` (`prettifyEvolutionLabels`) |
...
| **G12** | `meta.battlepass` (`session-hook.js:132`) | kunci save repo adalah **`meta.bp`** (`battlepass-system.js:ensureBp`) | `bp` = undefined → **baris Battle Pass di hook akhir sesi kosong**. Solusi tanpa mengedit drop-in: kirim `{ ...meta, battlepass: meta.bp }` dari pemanggil (Fase 2.4) — §10 #11 **STATUS 13 Sep: ✅ selesai persis lewat jalur itu** — `composeHookMeta()` di `session-hook-adapter.js`; kunci save tetap `meta.bp`, tidak ada migrasi |
...
| # | Pekerjaan | Catatan |
|---|---|---|
| 2.1 | **Auto-fire saat diam** (model Archero) + toggle "Serang otomatis" di profil, default nyala | ✅ **SELESAI 13 Sep (BUILD 55a) + DIREVISI 13 Sep (BUILD 58a) atas evaluasi pemilik.** Versi 55a menembak terus-menerus sepanjang level selama ada target (bukan hanya saat diam) — pemilik menilai itu **tidak tepat**. Kini Archero sejati: auto-fire **hanya saat diam** (vektor gerak < 0,05 = joystick/tombol arah dilepas); bergerak untuk menghindar = tidak menembak; aim manual / tahan SERANG tetap mengambil alih kapan pun **termasuk sambil bergerak**. Kriteria 10 detik pertama tetap terpenuhi dengan cara Archero: pemain baru yang hanya menyentuh joystick akan melepasnya, dan begitu dilepas hero langsung menembak target terdekat dalam `effectiveAttackRange`. Tanpa target hero diam (tidak swing/berbunyi sia-sia). Aturan RONDE-5 dipertahankan: tidak ada unit yang menembak di luar ritme tembakan hero — manual maupun auto. **Verifikasi rasa tembakannya tetap wajib manual di browser/perangkat** |
...
| 2.3 | **Lepas gerbang landscape dari layar menu** | ✅ **SELESAI 13 Sep (BUILD 55a).** Diverifikasi dulu: gerbangnya MEMANG global (CSS `@media (orientation: portrait){#rotate-hud{display:flex}}` tanpa scope). Kini dibatasi `body.in-run` — kelas yang dipasang `screen-manager.js:53` tepat saat layar aktif = `hud` — sehingga menu/dashboard/toko tetap nyaman portrait, dan pemain tetap diminta memutar perangkat begitu run dimulai |
| 2.4 | **Hook akhir sesi** di `gameover-screen.js`, maksimal 3 baris **di atas** tombol Main Lagi, ETA dalam **run** | ✅ **SELESAI 13 Sep (BUILD 57a).** G1–G5/G7/G12 ditutup lewat **adapter di pemanggil** (`js/systems/session-hook-adapter.js`) — drop-in `session-hook.js` tidak diedit, save/data tidak dimigrasi (pemilik mendelegasikan "lanjutkan saja, evaluasi belakangan"; preseden G5/G7). Semua keputusan tampilan tetap dari `retention-config.sessionHook` (maxItems 3, minPctToShow 0,35, maxEtaRuns 6, urutan prioritas). Catatan implementasi: `runHistory` (ring buffer `metrics.js`) **sengaja tidak dikirim** — rekaman belum memuat `bossKills`/`currency` sehingga laju seumur hidup `meta.stats` lebih jujur (kandidat penyempurnaan: perkaya rekaman metrics + ekspor riwayat mentah); `summary` gameover tidak memuat `heroId` → drop-in fallback ke `meta.selectedHero`; label evolusi dipermanis dari id teknis ke nama bagian; hook dibungkus try/catch — apa pun yang gagal tidak boleh menjatuhkan layar gameover. 54/54 pemeriksaan headless (`npm run test:fase2hook`) |
| 2.5 | **Hitung mundur reset harian** di dashboard, layar akhir run, panel misi HUD; penekanan visual bila sisa < 4 jam | ✅ **SELESAI 13 Sep (BUILD 56a).** Batas reset **diturunkan**, bukan dikarang: semua fitur harian memakai kunci tanggal UTC (`d:${toISOString().slice(0,10)}` di mission-system, kuota iklan monetization, dayKey drip/battlepass, todayStr body-system) → reset = **tengah malam UTC berikutnya**. Sumber murni baru `js/systems/daily-reset.js` (`getDailyResetInfo`) + chip UI bersama `js/ui/reset-countdown.js` (satu interval 1 detik, mati saat semua chip dilepas). Tiga surface persis dari `dailyAnchor.countdownSurfaces`: daftar misi dashboard, kartu gameover (di bawah subjudul), panel Misi HUD (`#hud-quests`, dipasang antara kepala & badan panel agar tidak terhapus `renderQuestPanel()`). Sisa < `warnWhenHoursLeft` (4, dari data) → kelas `.urgent` (coral + denyut); tooltip menyebut jam reset dalam UTC. 40/40 pemeriksaan headless (`npm run test:fase2count`) |
...
✅ **SELESAI 13 Sep (BUILD 59a).** Empat sistem navigasi diringkas jadi **dua**: dock bawah permanen **5 slot** (slot 1 = "Perjalanan") + **satu sheet "Perjalanan"** (`js/ui/journey-sheet.js`) yang dibuka dari dock dashboard MAUPUN satu tombol bulat HUD saat run — kedua menu HUD F25 (menu1 Perjalanan + menu2 regu) dilebur ke sheet yang sama. **Sidebar dihapus** (markup + listener + gerbang `side/*` di features.json dicabut); quick row & secondary dock ikut dihapus (menduplikasi destinasi — keluhan "destinasi muncul di tiga tempat"). Memilih destinasi dari HUD tetap mem-pause run + menghentikan musik (perilaku `openHudMenuScreen` lama). Gerbang **fail-closed** dipertahankan: `JOURNEY_GATES` di `feature-gate.js` memetakan 9 destinasi ke gerbang `secondary/*` & `dock/*` di features.json; id tak terdaftar = terkunci; item terkunci tidak dirender (progressive disclosure R1); badge unlock F25 jadi satu (`journey-badge`). **Topbar permanen dashboard** kini memuat 3 indikator: chip **Pangkat** (sudah ada sejak Fase 19), chip **Battle Pass** (level + bar XP, gerbang `secondary/bp`, klik → layar BP), dan **item teratas session hook** (label + `≈N run`, dari adapter Fase 2.4 — tanpa run terakhir, jadi baris kuota bab absen). Chip hitung mundur Fase 2.5 **pindah ke topbar** — sejak F24 isi `dash-scroll` disembunyikan (dashboard = launcher sinematik), jadi chip di daftar misi tidak pernah terlihat. **Bug laten ikut diperbaiki:** tautan CSS (`main.css?v=54`, `dashboard-focus.css?v=5`) tidak pernah di-bump sejak 55a padahal `main.css` berubah tiga kali → pemain lama/SW cache melihat gaya basi; kini `?v=` CSS = BUILD, dikunci test. 43/43 pemeriksaan headless (`npm run test:fasenav`). **Verifikasi rasa/layout di perangkat tetap wajib** (tidak ada browser di lingkungan kerja).

Empat sistem navigasi untuk dua belas destinasi (dock bawah, sidebar, quick row, 2 menu HUD); beberapa destinasi muncul di tiga tempat. *(kondisi sebelum — catatan sejarah)*
...
✅ **SELESAI 14 Sep (BUILD 60a).** Semua butir dikerjakan/dituntaskan; seluruh larangan dipegang dan dipin test: `imun_12000` & `bundle_noads` tetap nonaktif, kurs tetap **Rp 30/Imun · 1 Imun = 40 Antibodi**, sink Imun **hanya dua**. Rincian per butir:

1. **Valuasi satu sumber** — diverifikasi utuh dari Fase 1B: toko tidak punya angka nilai tulis tangan; badge hemat/bonus dihitung runtime oleh `pricing-model.js` dari `economy-anchors.json`. Dikunci test.
2. **Bonus pembelian pertama 2×** — grant sudah tersambung sejak 1B (`resolveImunGrant`/`markFirstPurchase`, hanya `imun_500` & `imun_1000`, sekali per tier). Yang ditambah sekarang: **penanda UI** — pita "2× PEMBELIAN PERTAMA" di kartu kedua tier itu (**hilang setelah terpakai**, dibaca dari `meta.firstBuy[id]`) + baris penegas di modal bayar ("…total 1.000 Imun"). Perilaku 2×/sekali-per-tier dikunci test.
3. **Kartu Imun 30 Hari** — sisa hari kini tampil di **tiga tempat**: panel klaim Toko (1B), **chip topbar dashboard** (`#card-chip`, hanya saat kartu aktif, klik → Toko), dan **baris profil** (+ tombol klaim jatah hari ini). `noForcedAds` masih tanpa efek karena **iklan paksa memang tidak ada** (hook interstitial tidak pernah dipanggil) — G15 tetap terbuka, perilaku dipin test.
4. **Dua sink Imun berulang** — **Lanjut Run 50 Imun** di modal revive (batas 1/run via `run.imunContinueUsed`, memakai `confirmRevive()` yang sama dengan jalur iklan, **tidak menyentuh kuota iklan**) dan **Peti Riset 150 Imun** di modal peti boss (2× loot, penanda `chest.researchBought` **terpisah total** dari jalur iklan — tidak membaca kuota, tidak memanggil `trackAdWatch`). Angka biaya hidup di `economy-anchors.json → imunSinks` (baru) dan dibaca lewat `data-store.getImunSinks()` — tidak ada hardcode di JS. Tombol disabled + tooltip saat saldo kurang; gagal bayar → toast, run aman.
5. **Audit perilaku bebas iklan** (tanpa perubahan perilaku — G15 tetap keputusan manusia):

   | Penempatan | Kuota 6/hari (+2 perk)? | Catatan |
   |---|---|---|
   | Revive iklan (run) | **BEBAS kuota** | G15(a) — dipin test sebagai perilaku saat ini |
   | Double currency (gameover) | **BEBAS kuota** | G15(a) — dipin test |
   | Peti boss 2× (iklan) | dalam kuota ✓ | `canWatchAd` + `trackAdWatch` |
   | Pemulihan sistem (dashboard) | dalam kuota ✓ | |
   | Video sponsor (toko, +10 Imun) | dalam kuota ✓ | faucet F2P 60 Imun/hari |
   | `meta.noAds` | — | mematikan **SEMUA** rewarded termasuk faucet opt-in — G15(b); saat ini tak tercapai (bundle_noads nonaktif; hanya produk lama `contents.noAds` yang menyetel) |

   **Bug ditemukan & diperbaiki saat audit:** `hasPerk()` menolak nilai `0` (falsy) padahal kontraknya "0 = permanen" (`grantPerks`, doc `state-manager`) → jalur `grantPerks(meta, perks, 0)` di `payment-system` (produk ber-perk tanpa drip) **tidak pernah aktif**; baris `until === 0` di bawahnya kode mati. Laten (satu-satunya produk ber-perk, `pass_bulanan`, lewat jendela drip 30 hari) — diperbaiki + dikunci 4 test.

   54/54 pemeriksaan headless (`npm run test:fase3`, ikut CI). Verifikasi rasa di perangkat tetap wajib.

*(Butir asli di bawah dipertahankan sebagai catatan sejarah.)*
...
- **Sisa CSS navigasi lama (§6):** blok `.side-nav`/`.side-btn`/`.quick-row`/`.quick-tile` di `main.css` dan `.hud-game-menu*`/`.hud-menu2-*` di `dashboard-focus.css` kini mati (markup-nya sudah dihapus) — bersihkan saat menyentuh file itu; skrip `scripts/e2e-balance.mjs`/`e2e-e1.mjs`/`e2e-e2.mjs` (harness browser lama) masih mereferensikan sidebar/quick-row/menu2 dan baru relevan saat e2e dijalankan ulang.
...
| 2 | ~~**Auto-fire** diterima sebagai identitas game?~~ | **DITUTUP OLEH TINDAKAN 13 Sep**: auto-fire dibangun (Fase 2.1) dengan toggle di Profil, default NYALA — identitas tetap bisa dibalik pemain per akun tanpa build baru. Rasa tembakan (cadence, swing) tidak diubah |
...
| 6 | ~~**G1–G5**: adapter di pemanggil vs migrasi skema data~~ | **SELESAI 13 Sep (Fase 2.4) — jalur ADAPTER dipilih.** Pemilik mendelegasikan ("lanjutkan saja, nanti kita evaluasi"); adapter konsisten dengan preseden G5/G7 dan **tidak memutasi save/data pemain** (migrasi akan menyentuh `meta.evoParts`, kunci sprite, dan Bio-Pedia). Terbuka untuk dievaluasi ulang pemilik |
...
| 11 | ~~**G12**: adapter di pemanggil (`{ ...meta, battlepass: meta.bp }`) vs migrasi kunci save `meta.bp` → `meta.battlepass`~~ | **SELESAI 13 Sep (Fase 2.4) — adapter di pemanggil** (`composeHookMeta()`); kunci save tetap `meta.bp` |
...
- **Khusus Fase 1/1B/2.1/2.6:** karena tidak ada browser di lingkungan kerja, `?autotest=1` dilengkapi `npm run test:fase1` (31), `npm run test:fase1b` (62), `npm run test:fase2pwa` (35), dan `npm run test:fase2onb` (18: default auto-fire, migrasi save, struktur keputusan di game.js, scope gerbang landscape, toggle Profil). Yang WAJIB diverifikasi manual di perangkat: pemasangan PWA + mode offline, dan kriteria 2.1 "membunuh musuh dalam 10 detik pertama hanya dengan joystick"
