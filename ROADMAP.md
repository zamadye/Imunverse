...
| Uji headless **31/31 lolos** tanpa browser (loader, save lama, comeback, streak, BP, GP, evolusi, kampanye) | `scripts/unit-fase1-retune.mjs` |
...
| **G13** | `grantCosmetic` menginisialisasi `meta.cosmetics = { owned: [], skin: null, … }` (`rare-drop-system.js:127`) | 3 tempat repo menginisialisasi **`skin: {}`** (objek): `battlepass-system.js:192`, `imun-economy.js:42`, `payment-system.js:86` | siapa yang jalan lebih dulu menentukan bentuk `skin` → **bentuk save tidak konsisten**. Solusi: inisialisasi kosmetik lewat jalur repo **sebelum** memanggil `grantCosmetic` (Fase 4.2) — §10 #12 |
...
|---|---|---|
| 1.1 save server-side | ⏳ **menunggu keputusan pemilik** | Supabase vs VPS vs tetap `localStorage`+PWA — tidak bisa diputuskan dari kode (§10 #10) |
| 1.2 telemetri | ⏳ **menunggu 1.1** (butuh endpoint) | `metrics.js` sudah menghitung one-more-run rate; 4 event belum dikirim |
...
| 10 | **Infrastruktur 1.1/1.2**: Supabase (auth + Postgres JSONB + RLS, cepat, biaya ikut MAU) vs VPS sendiri (kontrol penuh, butuh pemeliharaan) vs **tetap `localStorage` + PWA** sebagai keputusan sadar yang tercatat | **memblokir** 1.1, 1.2, dan seluruh KPI §13 |
...
- **Khusus Fase 1:** karena tidak ada browser di lingkungan kerja, `?autotest=1` dilengkapi `npm run test:fase1` (`scripts/unit-fase1-retune.mjs`, 31 pemeriksaan headless atas jalur boot nyata). Self-test browser tetap wajib sebelum rilis
