...
- [x] **11 hero sesuai tabel pemilik** (`look {build, accent, cape}` di heroes.json + trait renderer per hero): Mako besar mulut lebar perut buncit (hijau+kuning), Dendri visor+tentakel bercabang (oranye+emas), Neutron helm polisi+lencana+perisai punggung (biru laut+perak), Eos rambut gelombang+tombak (pink+salmon), Baso jas lab+tabung (ungu+biru), Mastia kokoh granula+perisai besar (marun+coklat), T-Bolt visor listrik+emblem petir+lengan-modif (biru listrik+perak), Helia mahkota+jubah+tongkat (emas+putih), Treg krag hakim+timbangan keadilan+perisai (zamrud+putih), Bella beret+celemek+kuas (ungu muda+pink), Nyx tudung ninja+mata redup+keris sitokin (hitam+ungu). Senjata (pedang/tombak/tongkat/kuas/keris) muncul di tangan; evolusi tetap terlihat (bahu + inti melayang).
- [x] **13 musuh humanoid monster** (`form` di enemies.json): spike (duri, virion merah #E74C3C), runner ramping condong (bakteri ungu #9B59B6/GN), tank perisai besar (GP hijau tua #2E7D32), splitter bergetar+retak (virus oranye #E67E22), boss SANGAT besar aura merah+mahkota duri (sel kanker/toksin raksasa); musuh melayang (toksin hazard) digambar tanpa kaki; wajah marah; animasi pukul saat menabrak player (`attackHint`).
- [x] **Pasukan (squad) humanoid mini** dengan warna sel masing-masing (Bella/Nyx/Mako/Eos/T-Bolt/Helia) — ring tim biru tetap.
- [x] **Avatar UI**: 11 potret diregenerasi sebagai **chibi humanoid** (kepala besar+tangan+kaki) — tampil di HUD kiri-bawah, roster, prep, detail hero; klik-kartu roster & alur lain tak berubah.
- [x] Bayangan ground dikecilkan & dirapatkan ke kaki agar figur "berdiri" natural di lantai perspektif.
- **Kriteria lulus 12d:** e2e **21 asersi PASS** (semua alur klik-riil tetap utuh), autotest **17/17**, perf sehat @41 musuh; tanpa perubahan logika gameplay (movement/attack/skill/HP/wave/save tidak disentuh).
