...
## Fase 20 — REBALANCE: XP Berat, Imun Coin Langka, Tier Hero Sejak Awal + Fix Layout & Modal (5 feedback pemilik) ✅
**Feedback pemilik (2026-09-06, 2 screenshot):** (1) kartu daily bonus/quests terlalu besar & bertumpuk di dashboard; (2) tombol Akhiri Run/hasil run terpotong — tak ada jalan kembali ke dashboard; (3) **paling fatal: naik level dalam hitungan detik** — harus ±3–5 menit per level, terus makin berat, modal level-up terus mengganggu; (4) Imun Coin terlalu mudah — kill musuh harusnya memberi point standar, IMU khusus item langka; (5) tier hero harus ditentukan **sejak awal** (common–legend, tak berubah oleh upgrade) — user dapat common–epic di awal; pasukan bertambah terus tapi musuh makin berat; hero kuat didapat dengan farm IMU di battle/dungeon atau tonton iklan/beli.
**Koreksi desain (semua angka dari data):**
1. **XP CURVE BERAT** — `xpCurve` 10·lv^1.5 → **280·lv^0.45** (upgrades.json): L2 = 383 XP (±43 kill kecil ≈ menit pertama), L5 = 578, L10 = 790 — naik terus beratnya, modal level-up tidak lagi menyela setiap detik. Band awal xpMult 1.6 → **1.35** (masih terasa murah, tidak instan).
2. **IMUN COIN = PREMIUM LANGKA** — `imuReward`: perWave 8→**0**, perKill 0.5→**0**; IMU hanya dari **boss (+20/boss)** & **bonus menang bab (+50)**. Chip HUD kini menampilkan **saldo total** (saldo + akruan run). Hadiah pendiri 300 → **150**. Misi reward IMU dipangkas ±70% (first_blood 60→20 … wave_20 400→120). Harga disusun ulang: skin dasar 45–60 IMU, skin langka (T-Bolt Krom) **400**, hero Legend (Nyx) **1500** — Nyx sengaja TIDAK terjangkau di awal (e2e membuktikan tombol BUKA Nyx state "poor").
3. **TIER HERO SEJAK AWAL** — `heroes.tiers` di data: Common ×1.0 (Mako), Uncommon ×1.07, Rare ×1.15, Epic ×1.24, **Legend ×1.35 (Nyx)** — ditulis di tiap hero (`hero.tier`), TIDAK berubah oleh upgrade. `computePlayerStats` mengalikan damage & HP dengan `statMult` tier. Badge rarity di kartu roster; banner detail hero menampilkan rarity (bukan lagi tier evolusi — evolusi murni tingkat kekuatan).
4. **PASUKAN TUMBUH** — jumlah anggota = max(bab-dibersihkan, **1 + floor(lvPasukan/3)**) (upgrades.allyUpgrade.membersPerLevels) — semakin naik level makin besar pasukan (maks 6), sementara kurva musuh F18 terus naik.
5. **FIX LAYOUT** — akar tumpukan: `grid-template-columns` play-row tanpa `minmax(0,…)` → min-content kartu Arena meluber ke kolom tile (bukti geometri: kartu sampai x=632, tile mulai 490). Fix: `minmax(0,1.12fr) minmax(0,1fr)` + `min-width:0/overflow:hidden` pada kartu. Tile quick-menu 2×3 **kompak** (`auto` rows), strip statistik mengisi sisa kolom kanan (tanpa lubang).
6. **FIX MODAL TERPOTONG** — kartu hasil run: `#gameover-summary` yang discroll (max-height 30vh), tombol Iklan 2x + Main Lagi + Dashboard **flex 0 0 auto** = selalu terlihat; gaya layar pendek ≤500px (stars 32px, cell ringkas, deco disembunyikan).
**Kriteria lulus 20:** e2e baru `scripts/e2e-balance.mjs` **13/13 PASS** (kurva dari data & L2≥250; 100 XP TIDAK menaikkan level pada run nyata; imuForRun(kill polos)=0, boss=60, menang=50, gabungan=90; tier table 11 hero lengkap; statMult Legend/Common terverifikasi lewat computePlayerStats; 11 badge tier dirender; pasukan 3 anggota di lv pasukan 6; tile ≤110px & tidak overlap play-row; tombol gameover di dalam viewport). Regresi penuh: inti **20/20**, retention **20/20** (Neutron 120 IMU yg dibeli klik-riil; Nyx langka), progression **17/17**, eco **14/14** (skin dasar 45), purpose **13/13**. Bukti `shots/81-dash-tiles-compact, 82-gameover-fit, 83-roster-tier-badges`. Buster `v=20a`.

## Fase 19 — TUJUAN PEMAIN: Pangkat Penjaga + Home Full-Bleed (feedback pemilik) ✅
**Masalah pemilik (2026-09-06, screenshot HP):** (1) ruang kosong di bawah kartu battle pass/quick-menu; (2) carousel sebaiknya mentok kiri-kanan (lebih enak dilihat); (3) dock/topbar terlalu sempit untuk layar landscape — dan yang terpenting: **"tanpa tujuan yang jelas, trigger pun tidak kuat"** — MLBB/FF/PUBG menyentuh emosi lewat pengakuan, kebanggaan rank, kepemilikan (skin).
**Referensi internet (riset 2026-09):** [esportsinsider — MLBB ranks] tier ladder memberi "sense of progression & something to work towards", rank pemula TANPA penalti; [Moller dkk., selfdeterminationtheory.org — kompetisi di game digital] demosi/penalti menggerus motivasi intrinsik (anak kasual stres) — Duolingo menyelesaikannya dengan kelompok sebaya & tanpa hukuman turun; [Udonis — leaderboard design] recognition + visible progress = motivasi inti; SDT: competence/autonomy/relatedness.
1. **PANGKAT PENJAGA (tujuan emosional)** — 13 tier ladder dari `data/ranks.json` (Sel Baru → Patroli Imun I–III → Penjaga Jaringan I–III → Komando Limfa I–III → Elite Antibodi → Makro Legenda → **Penjaga Sejati**) dengan warna & lambang per tier. **GP dari SETIAP run** (menang maupun kalah — usaha anak tetap dihargai): wave ×18 + kill ×2 + boss ×120 + menang +250 + bab +250; semua angka dari data. **TANPA DEMOSI** (ramah anak, ala rank Warrior MLBB): GP hanya naik. **Musim 28 hari**: ganti musim = soft reset ke lantai tier (pangkat tidak pernah turun) — urgensi ala season CoD:M tanpa rasa rugi. GP **tidak bisa dibeli** (fair, selaras premium cap 30%).
2. **Titik temperamen emosi**: chip pangkat selalu terlihat di topbar dashboard (goal-gradient: bar progres + "N GP lagi ke [tier]"); modal **PANGKAT PENJAGA** (klik chip riil): emblem besar, bar progres, ladder 13 tier (saat ini disorot emas, tercapai dicentang), garis musim "berakhir dalam N hari", rincian cara dapat GP dari data; kartu hasil run menampilkan "+N GP"; **ceremony NAIK PANGKAT** emas dengan fanfare + animasi pop "Patroli X → Patroli Y".
3. **Layout home full-bleed** (3 permintaan UI): (a) carousel banner **mentok kiri-kanan** (tembus padding layar, tanpa radius) tinggi clamp(176px, 46vh, 236px) — baris Play tetap mengintip di fold; (b) **ruang kosong tertutup**: quick-menu pindah ke samping kartu kampanye (grid 2×3 meregang penuh setinggi kartu — tanpa lubang), strip statistik & daily jadi baris full-width; (c) **dock & topbar melebar penuh** (ala MLBB landscape), sidebar dipadatkan agar 6 tombol muat di layar pendek.
**Kriteria lulus 19:** e2e baru `scripts/e2e-purpose.mjs` **13/13 PASS** (klik riil: chip→modal, ladder 13 + sorot current, garis musim, tutup modal; GP dihitung dari run nyata — kalah pun dapat; ceremony NAIK PANGKAT tampil & chip terpromosi; EN mengubah seluruh modal). Regresi penuh: inti **20/20**, retention **20/20** (deterministik: musuh dijaga dalam jangkauan melee — tes lama borderline-lucky, auto-attack tetap riil), progression **17/17** (gate test kini menunggu kondisi, tahan beban), eco **14/14** — ERRORS: none. Bukti `shots/78-dash-fullbleed-rankchip, 79-rank-modal, 80-rankup-ceremony`. Buster `v=19a`; lang.json 670 string.

## Fase 18 — Kurva Progresi & Gerbang Penjaga (Gatekeeper) ✅
Semua angka kurva hidup di **data/progression.json** (baru, sv2) — tanpa angka keras di logic; `data/waves.json` tidak berubah.
1. **3 BAND KESULITAN** (diterapkan di dalam getter `data-store` — satu sumber kebenaran, tanpa duplikasi di spawn-system): **Awal wave ≤5** ramai & murah (spawn ×1.3, HP musuh ×0.8, XP ×1.6, reward ×1.5) — awal main cepat naik, reward terasa besar; **Tengah ≤15** netral ×1.0 (upgrade mulai penting); **Akhir 16+** berat (spawn ×0.78, HP ×1.55, musuh lebih gesit +0.10, XP ×0.85, reward ×1.3 — hanya build optimal yang bertahan).
2. **SCALING MUSUH IKUT LEVEL PLAYER** — setiap spawn: HP ×(1+(level−1)·6%), speed +(level−1)·0.4% (cap +12%) dari `playerLevelScaling` — musuh selalu sepadan, tidak ada instan gampang setelah install.
3. **GERBANG PENJAGA (spec: wave & naik-level BERHENTI sampai boss dikalahkan)** — boss tiap 5 wave menutup gerbang: **timer wave BEKU**, spawn reguler jadi **trickle ×2.2**, pill merah HUD "⚑ KALAHKAN PENJAGA!" berdenyut; boss tumbang → announce "GERBANG TERBUKA!" + wave lanjut.
4. **XP BANK (bank XP)** — selama gerbang tertutup, semua XP (orb, wave bonus, dsb.) **DITAHAN** di `run.xpBank` tanpa progres level (chip kuning "+N XP ditahan" di pill gerbang); penjaga tumbang → **cair penuh** (`gatekeeper.xpBankRatio = 1.0`) — satu momen ledakan level-up di belakang boss, modal pilihan mengantre rapi.
5. **PREMIUM TETAP +30% MAKS** — `premiumPowerCapPct: 30` (serum +25%, koin_ganda diturunkan 50%→**+30%**) — tidak pay-to-win; tanpa premium tetap clearable dengan effort lebih.
**Kriteria lulus 18:** e2e baru `scripts/e2e-progression.mjs` **17/17 PASS** (band awal lebih mudah dari tengah; akhir lebih padat & HP lebih keras; HP wave-3 = (1+2·0.105)·0.8 & wave-20 = (1+19·0.105)·1.55 persis; scaling ikut level player 1→4; XP band awal ×1.6; gerbang terkunci di wave 5 + pill HUD tampil + timer beku; **XP masuk bank tanpa naik level & cair setelah boss tumbang**; serum 25% & koin 30% ≤ cap — semua klik riil). Regresi: e2e inti **20/20** (5× beruntun — tes tap SERANG & skill kini deterministik), retention **20/20**, eco **14/14** — ERRORS: none. Bukti `shots/75-gate-pill, 76-gate-xp-bank, 77-gate-terbuka`.
...
## Fase 21 — Gameplay-First Onboarding: Main Dulu, Menu Kemudian (9 feedback pemilik + referensi indie) ✅
**Feedback pemilik (9 screenshot indie — Crownstone Survival, Magic School Story):** pemain baru disambut LANGSUNG gameplay (bukan dashboard); alur awal = cerita sinematik per scene → gameplay; tutorial cara main in-game; dashboard terbuka BERTAHAP via progres (skala jam/hari, bukan menit); modal mendadak (level-up) jadi scene sinematik bercerita; dock tidak mendominasi ±30% layar; halaman non-home full-width landscape.
- [x] **Title screen gameplay-first** (`#screen-title`): MULAI → sinematik 'onboarding' (6 shot cerita Mako si makrofag: virus menyusup lewat makanan → menyebar → Mako bangun → kamu adalah Mako → kontrol → evolusi/PENJAGA) → **langsung `startRun` bab pertama tanpa prep/dashboard**; "Sudah punya akun? Masuk" → auth; pill EN/ID. Boot: `!hasAccount() → 'title'` (sinematik 'intro' hanya pemain lama); Coach dashboard hanya pemain lama.
- [x] **Tutorial in-game run pertama**: tutorial-system 3 langkah (bubble "Tarik di layar…" + misi bersihkan 25 patogen + kartu Bio-Pedia) — sudah ada, kini SELALU aktif di run pertama onboarding.
- [x] **Gerbang menu bertahap** `data/features.json` + `js/systems/feature-gate.js`: kunci = `meta.stats.bestWave` (ditulis `finishRun`, game.js) → dock Heroes Gel.2/Bag Gel.4/Squad Gel.6/Shop Gel.8; quick tiles (Misi 2, Bag 4, Toko 8, Codex 1, Pass 6); sidebar (Codex 1, Tubuh 2, Rekor 3, Pass 6); Play/Home selalu terbuka. Klik terkunci → toast "Capai Gelombang N…" (+ EN). Visual: ikon redup + pil "🔒 Gel.N" (dock di samping ikon, sidebar pojok kanan-atas).
- [x] **Akun setelah run pertama**: `__IMUNVERSE_goDashboard` gate akun — akhir run onboarding → tombol Home/Play → auth dulu (progres run tersimpan ke akun).
- [x] **Level-up = scene sinematik**: modal jadi kartu panggung `.lu-scene` (glow, potret hero `#levelup-hero`, sub bercerita "Level N — <Hero> beradaptasi! Pilih evolusi:", kartu pilihan stagger 0.28/0.40/0.52 s; fix `backwards`→`forwards` agar kartu tak menghilang setelah animasi).
- [x] **Dock compact**: padding 4px, ikon 42px, label 9.5px — tinggi 62px @390 (dari ±30% → ±16% tinggi layar).
- [x] **Full-width landscape**: roster/premium/shop/bag/skin/codex/bp `max-width:none` auto-fill.
- **Kriteria lulus:** `scripts/e2e-onboarding.mjs` **15/15 PASS** (title-first, i18n EN=START, sinematik onboarding + teks cerita, langsung-gameplay tanpa dashboard/auth, tutorial in-run, level-up scene + sub cerita, akun wajib pasca-run, dock & quick tiles tergerbang + klik diblokir, dock 62px, pemain lama tidak lihat title, 0 pageerror) + regresi penuh **CORE 20/20, RET 20/20, ECO 14/14, BAL 13/13, PROG 17/17, PURPOSE 13/13**; bukti `shots/84-title-first, 85-onboarding-story, 86-straight-gameplay, 87-levelup-scene, 88-account-after-run, 89-dash-gated`.
- **Terbuka (menunggu konfirmasi pemilik):** desain inti musuh — musuh indie "berhenti = aman" vs desain kini musuh terus mengejar; TIDAK diubah tanpa persetujuan.

## Fase 22 — Adopsi Desain Pemilik arena/01a07727: Combat-First Gameplay UI (baseline baru) ✅
**Arahan pemilik:** "clone branch terbaru arena/01a07727 — design yang mendapat kemauan saya; fokus kali ini gameplay UI/UX, bukan menu dashboard (jalur lama salah arah)."
- [x] **Adopsi penuh** (merge `8d27056` → tree identik dengan branch pemilik): HUD v2 combat-first (top-left kills, center wave/timer mini, bottom-left portrait+HP/XP tanpa kartu, bottom-right 2 skill + SERANG, kanan-atas pause+menu ☰ Campaign/Bio-Pedia/Rank); **wave berfase** spawn → fase CLEAR ("ARENA BERSIH · WAVE N", spawn berhenti) → arena bersih → jeda ±2.5 dtk → wave baru; **Mako ranged pierce** (attackRange 300, projectile 280 — auto-attack terasa); meta level keluar dari HUD; **quest harian/mingguan accept-claim** (AMBIL → AKTIF → KLAIM, Antibodi); dev mode `?dev=1` (unlock in-memory, save asli aman); layar Profil (klik chip akun); dashboard CTA **PLAY** besar + dock utilitas 4 (Heroes/Shop/Collection/Stats, tergerbang F21); secondary-dock & sidebar disembunyikan desain pemilik; hero unlock diselaraskan (Neutron 100/Dendri 150/Nyx 200; unlock statistik dinaikkan: 15 run, Gel.20, 500 kill).
- [x] **Butir #8 (musuh) TERJAWAB oleh desain pemilik**: tekanan wave kini ber Ritme — bukan arus tanpa akhir; musuh berhenti ditambah saat fase break (pemain bernapas & mengumpulkan nutrisi). (Leash "berhenti=aman" per-musuh TIDAK diadopsi; arah pemilik = fase wave.)
- [x] **Fix integrasi**: i18n string baru (Mulai run sekarang → "Start a run now"; ARENA BERSIH → "ARENA CLEAR"; chip "Lv N · Pasukan Lv N·M sel" → "Squad Lv N · M cells") + buster `12b-3`; suite ECO/BAL/PROG diadaptasi ke desain baru (BP via API karena nav BP kini tanpa pintu UI — **gap dilaporkan**: secondary-dock disembunyikan, Battle Pass & Rank & Campaign hanya via menu ☰ in-game; quick-row kini 4 tile; uji wave clear→break 3 fase).
- **Kriteria lulus:** ONBOARD 15/15, CORE 20/20, RET 20/20, ECO 14/14, BAL 13/13, PROG 18/18, PURPOSE 13/13 — semua di desain baru; bukti `shots/90-hudv2-early, 91-wave-break, 92-hud-menu, 93-hudv2-mid, 94-levelup-scene, 95-dashboard-focus, 96-dashboard-en`; tree = identik branch pemilik + fix i18n/suite.
- **Gap untuk keputusan pemilik:** (1) Battle Pass tidak punya pintu masuk UI di dashboard (klaim masih bisa, masuk via API/menu; tambah ikon di suatu tempat?); (2) tombol suara & reset save dihapus dari dashboard oleh desain baru (suara masih ada di pause); (3) rank & campaign kini hanya dijangkau dari menu ☰ saat gameplay atau banner dashboard.

## Fase 23 — Profil = Pusat Pengaturan + Musik Prosedural + Battle Pass ke Menu Gameplay ✅
**Arahan pemilik (F23):** suara ditaruh di PROFIL (bukan settings terpisah — "letak umumnya memang di dalam settings/profil"); tambahkan musik non-copyright; fix gap #1 = Battle Pass harus punya pintu masuk (ditaruh di menu ☰ gameplay karena memang semua menu akan hidup di dalam gameplay — F25).
- [x] **Musik latar prosedural** `js/systems/music-system.js`: melodi chiptune ceria 8-bar (C–G–Am–F) + bass + pad + arpeggio disintesis WebAudio saat runtime → **100% bebas lisensi** (bukan rekaman; slot tukar file musik pemilik tinggal mengganti playStep). Scheduler lookahead anti-jitter, volume lembut di bawah SFX, berbagi AudioContext dgn audio-system. Start saat `runstart`, stop saat gameover/keluar arena via menu ☰. Toggle `meta.musicOn` (default AKTIF).
- [x] **Pengaturan di Profil**: baris MUSIK & EFEK SUARA (AKTIF/MATI, klik riil, persist ke save — tervalidasi reload) + **Reset Save pindah ke Profil** (menutup gap tombol reset yang dihapus desain 01a07727). i18n: Pengaturan/Musik/Efek Suara/MATI (+EN); buster `23a`.
