...
| `BUILD` `50a → 51a → … → 57a`, `index.html ?v=57a` | aturan cache-busting repo |
...
| **2.1+2.3 ✅ Onboarding** — auto-fire default NYALA (model Archero: hero menembak musuh terdekat dalam jangkauan tanpa input tembak; aim manual/tahan SERANG tetap mengambil alih; pasukan mengikuti ritme tembakan hero; tanpa target hero diam) + toggle "Serang Otomatis" di Profil; gerbang landscape kini **hanya** saat gameplay (`body.in-run`) | `game.js`, `profile-screen.js`+`index.html`, `hud-screen.js` (hint adaptif), `main.css`, `state-manager.js` (`settings.autoFire`), 18 pemeriksaan headless |
...
| # | Pekerjaan | Catatan |
|---|---|---|
| 2.1 | **Auto-fire saat diam** (model Archero) + toggle "Serang otomatis" di profil, default nyala | ✅ **SELESAI 13 Sep (BUILD 55a).** Catatan implementasi: auto-fire aktif selama ada target dalam `effectiveAttackRange` (bukan hanya saat diam) karena kriteria terima brief menuntut pemain yang hanya menyentuh joystick tetap bertarung; tanpa target hero diam supaya tidak swing/berbunyi sia-sia. Aturan RONDE-5 dipertahankan dalam bentuk baru: tidak ada unit yang menembak di luar ritme tembakan hero — manual maupun auto. **Kriteria 10 detik pertama wajib diverifikasi manual di browser** (tidak ada browser di lingkungan kerja) |
