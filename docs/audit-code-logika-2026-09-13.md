...
> **STATUS PERBAIKAN (2026-09-13):** K1, T1, T2, S1, S2, L1, L2, L3, L4, L5, L8, L9, L10 — **DIPERBAIKI** (BUILD 51a → 52a). Ronde-2 (audit lanjutan sistem kill-path + layar uang): F1, F2, F5 **DIPERBAIKI** (BUILD 53a); F7/F8/F9 diputuskan user 2026-09-13 lalu **DIPERBAIKI** (BUILD 54a: 2 jalur Imun [video sponsor 5×/24 jam utk gratis, bundle langsung utk buyer], semua siklus harian = 24 jam ROLLING per akun, semua transaksi toko wajib akun). L6/L7/L11/L12/L13 tetap catatan (disengaja/laten). Bukti: `npm run check` hijau, `node --check` semua file tersentuh, unit test walk-anim 23/23, e2e-retention `imu-not-from-plain-kills` konsisten dengan fix T2. Catatan: `mastery-system.js` — reward Mastery = **Antibodi** (RONDE-4), toast & header yang salah menulis "Imun" ikut diperbaiki.
...
| F7 | 🟠 TINGGI | `shop-screen.js` tile Suplemen Premium | **IAP diblokir kuota iklan** & tercatat sebagai tontonan iklan (`canWatchAd`/`trackAdWatch`): pemilik "Bebas Iklan" justru **tidak bisa membeli** suplemen. **DIPUTUSKAN USER 2026-09-13:** IAP lepas dari gating kuota iklan; 2 jalur resmi premium — (a) **user non-payer: dapat Imun Coin dari video sponsor, 5× per 24 jam**, (b) **user buyer: beli langsung Imun Coin / item di shop**. **FIX:** gate `canWatchAd`+`trackAdWatch` dihapus dari tile IAP; tile video sponsor kini grant **Imun Coin** (`battlepass.offers.adImun=30`, `adDailyLimit` 6→**5** di `upgrades.json`); bundle **Paket Imun** (100 Imun, Rp 25rb) ditambahkan ke `premium.json`; panduan currency (main.js) ditulis ulang. | ✅ DIPERBAIKI |
| F8 | 🟡 SEDANG | `mission-system.js` / `economy-system.js` / `body-system.js` / `monetization.js` | Kunci periode harian campur UTC/lokal → batas reset berbeda antar sistem. **DIPUTUSKAN USER 2026-09-13:** refresh = **24 jam ROLLING per akun** — "kalau klaim jam 8, jam 8 pula refresh-nya, bebas tiap user; yang penting 24 jam / 1×". **FIX (semua sistem disamakan):** daily-reward (`lastDailyClaimTs`), quest harian/mingguan (anchor 24 jam / 7 hari, migrasi schema `periodKey`), kuota iklan (`adDaily.anchorTs`), decay tubuh + rawat + streak sehat (`lastDecayTs`/`lastCaredTs`/`lastPerfectTs`, fase siklus dipertahankan). Save lama dimigrasikan otomatis; anti double-claim tetap. Terverifikasi unit test node (rolling 24j/7 hari, migrasi, anti double-claim). | ✅ DIPERBAIKI |
| F9 | 🟡 SEDANG | `shop-screen.js` vs `account-system.js` | Gating akun tidak konsisten (kosmetik Imun & suplemen tanpa `requireAccount`). **DIPUTUSKAN USER 2026-09-13:** "ya, harus pakai akun baru bisa payer" → **semua transaksi toko wajib akun**. **FIX:** `requireAccount('shop')` ditambahkan ke suplemen (Antibodi), Suplemen Premium (IAP), dan pembelian skin & aksesori. (Pembayaran bundle sudah wajib akun di `payOrder`.) | ✅ DIPERBAIKI |
