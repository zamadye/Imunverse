...
![genre](https://img.shields.io/badge/genre-roguelike%20survival-35d0ba) ![tech](https://img.shields.io/badge/tech-vanilla%20JS%20%2B%20Canvas%202D-4cc9f0) ![build](https://img.shields.io/badge/BUILD-60a-f5c64f)
...
- **Auto-fire (BUILD 55a, direvisi 58a):** model Archero sejati — hero menembak sendiri ke musuh terdekat dalam jangkauan **hanya saat diam** (joystick/tombol arah dilepas); bergerak untuk menghindar = tidak menembak. Default **NYALA**, toggle "Serang Otomatis" di Profil; aim manual / tahan SERANG mengambil alih kapan pun termasuk sambil bergerak, dan pasukan mengikuti ritme tembakan hero. Revisi 58a atas evaluasi pemilik: versi 55a menembak terus-menerus sepanjang level dan itu tidak tepat. Gerbang "Putar HP-mu" hanya muncul saat gameplay (`body.in-run`) — menu dan dashboard tetap nyaman portrait.
...
- **Fase 3 monetisasi tuntas (BUILD 60a):** penanda **"2× pembelian pertama"** di tier `imun_500`/`imun_1000` (hilang setelah terpakai); **sisa hari Kartu Imun 30 Hari** tampil sebagai chip topbar dashboard + baris profil dengan klaim jatah; dua **sink Imun** berulang: **Lanjut Run 50 Imun** di modal tumbang (1×/run, bangkit persis seperti jalur iklan, tidak memakan kuota iklan) dan **Peti Riset 150 Imun** di peti boss (2× loot tanpa iklan, penanda terpisah dari jalur gratis) — biaya hidup di `data/economy-anchors.json → imunSinks`. Audit perilaku iklan dibukukan di ROADMAP §7 (G15 tetap keputusan manusia) + bug fix `hasPerk` (perk permanen tak pernah dikenali). Kurs tidak berubah, `imun_12000`/`bundle_noads` tetap nonaktif — semuanya dipin 54 test headless (`npm run test:fase3`).
- **Navigasi dua sistem (BUILD 59a, §6):** dock bawah permanen **5 slot** (slot 1 = **Perjalanan**) + **satu sheet "Perjalanan"** yang sama untuk dashboard dan HUD — sidebar, quick row, secondary dock, dan dua menu HUD lama dihapus/dilebur. 9 destinasi (Peta Tubuh, Pangkat, Bio-Pedia, Battle Pass, Heroes, Arena, Lab Pasukan, Shop, Tas) semuanya lewat gerbang **fail-closed** `data/features.json` (`journeyGate`): item terkunci tidak dirender, id tak terdaftar terkunci selamanya; memilih destinasi dari HUD mem-pause run + menghentikan musik. Topbar dashboard memuat 3 indikator permanen: **chip Pangkat**, **chip Battle Pass** (level + bar XP), dan **item teratas session hook** ("… · ≈N run"); chip hitung mundur reset harian pindah ke topbar agar benar-benar terlihat (isi dashboard tersembunyi sejak F24). Tautan CSS kini ikut `?v=BUILD` (sebelumnya statis — gaya basi bagi pemain lama).
