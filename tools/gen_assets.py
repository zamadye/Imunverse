#!/usr/bin/env python3
"""
gen_assets.py — Pipeline aset non-karakter Imunverse v2.

Character Agent owns hero/enemy/pathogen sprites in tools/gen_sprites.py.
File ini tidak lagi menulis hero/enemy atau overlay evolusi lama supaya desain
karakter baru tidak tertimpa generator historis.

Masih digenerate di sini: ikon UI, FX kontrol, nutrisi, arena/thumb, dekorasi,
sistem tubuh, dan ikon part equity yang dipakai data/evolutions.json.
...
def gen_stage_heroes_v2(out):
    """Legacy hook: hero sprites now belong to tools/gen_sprites.py."""
    print("SKIP — hero Character Agent assets are generated by tools/gen_sprites.py")
    return {}


def gen_stage_portraits_v2(out):
    """Legacy hook: hero portraits now belong to tools/gen_sprites.py."""
    print("SKIP — portrait Character Agent assets are generated by tools/gen_sprites.py")
    return {}
...
def gen_stage_enemies(out):
    """Legacy hook: enemy/pathogen sprites now belong to tools/gen_sprites.py."""
    print("SKIP — enemy Character Agent assets are generated by tools/gen_sprites.py")
    return {}
...
# =====================================================================
# STAGE 9 — EQUITY HERO, KEMAMPUAN AKTIF & ARENA (14 aset)
# =====================================================================
...
def part_equity_receptor(size):
    """Equity part: Kode Reseptor — membuka cue anatomi pertama tiap hero."""
    img, d, S = _part_canvas(size, (99, 199, 106, 255))
    for rot in (-0.65, 0, 0.65):
        cx, cy = S * 0.5, S * 0.54
        pts = [(0, S * 0.20), (0, -S * 0.04), (-S * 0.16, -S * 0.24), (0, -S * 0.04), (S * 0.16, -S * 0.24)]
        cr, sr = math.cos(rot), math.sin(rot)
        xy = [(cx + x * cr - y * sr, cy + x * sr + y * cr) for x, y in pts]
        d.line(xy[:3], fill=(255, 243, 176, 255), width=max(3, int(S * 0.045)), joint="curve")
        d.line([xy[3], xy[4]], fill=(255, 243, 176, 255), width=max(3, int(S * 0.045)))
    d.ellipse([S * 0.43, S * 0.42, S * 0.57, S * 0.56], fill=(18, 63, 58, 210))
    return done(img, size)


def part_equity_membrane(size):
    """Equity part: Modul Membran — bahan organela/siluet tahap kedua."""
    img, d, S = _part_canvas(size, (74, 163, 224, 255))
    for rr in (0.18, 0.28, 0.38):
        d.ellipse([S * (0.5 - rr), S * (0.5 - rr), S * (0.5 + rr), S * (0.5 + rr)], outline=(196, 236, 255, 255), width=max(2, int(S * 0.026)))
    d.line([(S * 0.28, S * 0.5), (S * 0.72, S * 0.5)], fill=(255, 255, 255, 210), width=max(3, int(S * 0.04)))
    return done(img, size)


def part_equity_effector(size):
    """Equity part: Enzim Efektor — payload ofensif/defensif per hero."""
    img, d, S = _part_canvas(size, (176, 122, 224, 255))
    d.line([(S * 0.34, S * 0.72), (S * 0.64, S * 0.28)], fill=(255, 255, 255, 255), width=max(4, int(S * 0.06)))
    d.polygon([(S * 0.64, S * 0.18), (S * 0.73, S * 0.34), (S * 0.59, S * 0.32)], fill=(255, 209, 90, 255))
    for ang in range(0, 360, 72):
        a = math.radians(ang)
        x = S * 0.67 + math.cos(a) * S * 0.16
        y = S * 0.27 + math.sin(a) * S * 0.16
        d.line([(S * 0.67, S * 0.27), (x, y)], fill=(255, 209, 90, 235), width=max(2, int(S * 0.025)))
    return done(img, size)


def part_equity_memory_core(size):
    """Equity part: Inti Memori — komponen Full Equity."""
    img, d, S = _part_canvas(size, (245, 198, 79, 255))
    d.polygon([(S * 0.5, S * 0.16), (S * 0.76, S * 0.42), (S * 0.66, S * 0.78), (S * 0.34, S * 0.78), (S * 0.24, S * 0.42)], fill=(255, 243, 176, 230))
    d.ellipse([S * 0.39, S * 0.39, S * 0.61, S * 0.61], fill=(122, 73, 4, 215))
    d.ellipse([S * 0.16, S * 0.16, S * 0.84, S * 0.84], outline=(255, 224, 130, 235), width=max(2, int(S * 0.025)))
    return done(img, size)
...
def gen_stage_evo(out):
    print("STAGE 9 — Equity, kemampuan & arena (14 aset)")
    return {
        "part_equity_receptor.png": part_equity_receptor(112),
        "part_equity_membrane.png": part_equity_membrane(112),
        "part_equity_effector.png": part_equity_effector(112),
        "part_equity_memory_core.png": part_equity_memory_core(112),
...
def main():
    ...
    for stage in stages:
        ...
        for name, img in sprites.items():
            ...
            img.save(path, "PNG")
            total += 1
    print(f"\nTOTAL: {total} aset PNG non-karakter tersimpan di {os.path.abspath(OUT)}")
    # Character Agent cleanup: hapus hanya aset evolusi/hero lama yang memang
    # sudah diganti. Jangan sweep seluruh folder karena hero/enemy sekarang
    # dimiliki tools/gen_sprites.py dan data JSON.
    legacy = {
        "ov_silia.png", "ov_pseudopodia.png", "ov_pedang.png", "ov_inti.png",
        "part_silia.png", "part_pseudopodia.png", "part_pedang.png", "part_inti.png",
        "hero_sel_t_idle.png", "hero_sel_t_attack.png",
        "hero_sel_b_idle.png", "hero_sel_b_attack.png",
        "hero_sel_nk_idle.png", "hero_sel_nk_attack.png",
        "hero_makrofag_idle.png", "hero_makrofag_attack.png",
        "hero_eosinofil_idle.png", "hero_eosinofil_attack.png",
        "hero_neutrofil_idle.png", "hero_neutrofil_attack.png",
        "portrait_sel_t.png", "portrait_sel_b.png", "portrait_sel_nk.png",
        "portrait_makrofag.png", "portrait_eosinofil.png", "portrait_neutrofil.png",
    }
    for f in sorted(legacy):
        path = os.path.join(OUT, f)
        if os.path.exists(path):
            os.remove(path)
            print(f"  ✚ dihapus legacy Character Agent: {f}")
